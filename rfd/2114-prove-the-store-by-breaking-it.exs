...
#
# RFD 2114. `mix rfd.render` in rfd_dsl/ renders rfd/2114-prove-the-store-by-breaking-it/README.md and
# DETAILS.md from this file; the Markdown is a build artifact (RFD 2232).
defmodule RFD2114 do
  use RFD.DSL

  rfd 2114, "Prove the store by breaking it" do
    state :prediscussion
    ...
    details_title "Prove the store by breaking it"

    details "Trap one: a backup URL that is not restorable, and does not say so", ~S"""
    `fdbbackup start -d file:///backup/` does not write the backup to that
    path. It makes a container under it, `backup-<timestamp>/`, and writes
    there.
    ...
    details "Trap two: a key count that reads as total data loss", ~S"""
    ...
    details "The negative control", ~S"""
    ...
    details "A transaction that is too large for a new cluster", ~S"""
    ...
    details "What one machine cannot test", ~S"""
