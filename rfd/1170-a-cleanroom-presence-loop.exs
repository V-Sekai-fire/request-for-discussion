...
#
# RFD 1170. `mix rfd.render` in rfd_dsl/ renders rfd/1170-a-cleanroom-presence-loop/README.md and
# DETAILS.md from this file; the Markdown is a build artifact (RFD 2232).
defmodule RFD1170 do
  use RFD.DSL

  rfd 1170, "A cleanroom presence loop, and which components swap" do
    state :ideation
    ...
    **Two stages swap for reasons already held here.** Parakeet is
    ...
    details_title "A cleanroom presence loop, and which components swap"

    details "Why cleanroom, and what that means here", ~S"""
    ...
    **That distinction is worth holding even when nobody is watching.**
    ...
    details "The pipeline, as the author documents it", ~S"""
    ...
    **Only the glue is unlicensed.** That is what makes a rebuild cheap
    ...
    details "Convergence on the TTS, and how much weight it carries", ~S"""
    ...
    details "The two swaps, and why each is a swap rather than a preference", ~S"""
    ...
    **A hosted API is not blocklisted for inference.** CLAUDE.md blocks
    ...
    details "The lip-sync, which is the genuinely new part", ~S"""
    ...
    **The mapping's weight numbers are placeholders**, cited in the file's
    ...
    details "The body half, which that Space does not have at all", ~S"""
    ...
    **SOMA-X to ANNY is already the sanctioned route**, and the blocklist
    ...
    details "A LoRA on Kimodo is a much smaller thing than the retrain", ~S"""
    ...
    **So a LoRA is permitted, the base is licence-clean, and the data can be
    ...
    details "Can Mitsuba 3 render this in real time, and the answer is at a head", ~S"""
    ...
    **One number in that table should be distrusted, and it is stated
    ...
    details "Decided: Mitsuba is the reference renderer, Godot is the runtime", ~S"""
    ...
    **Its strength is the job this workspace already gives it.** RFD 1166
    ...
    details "What to do, cheapest first", ~S"""
    ...
    details "Measurement pass, 2026-09-04", ~S"""
