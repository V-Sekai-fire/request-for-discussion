...
#
# RFD 2109. `mix rfd.render` in rfd_dsl/ renders rfd/2109-two-tiers-with-foundationdb-as-the-store/README.md and
# DETAILS.md from this file; the Markdown is a build artifact (RFD 2232).
defmodule RFD2109 do
  use RFD.DSL

  rfd 2109, "Two tiers with foundationdb as the store" do
    state :prediscussion
    ...
    details_title "Two tiers with foundationdb as the store"

    details "Egress is the cost, and the store is not", ~S"""
    Bandwidth per client sets the bill. `rfd/0100` caps a client at 256
    kbps, and egress scales with concurrent clients and hours played. No
    deployment runs, so no monthly figure exists.
    """

    details "The database is not what the money buys", ~S"""
