...
#
# RFD 2202. `mix rfd.render` in rfd_dsl/ renders rfd/2202-rebac-bao-enforcement/README.md and
# DETAILS.md from this file; the Markdown is a build artifact (RFD 2232).
defmodule RFD2202 do
  use RFD.DSL

  rfd 2202, "ReBAC role tuples enforced via Bao identity groups" do
    state :committed
    ...
    details_title "ReBAC role tuples enforced via Bao identity groups"

    details "Role → group → policy mapping", ~S"""
    ...
    details "What Bao enforces after this RFD lands", ~S"""
    ...
    details "What Bao does not enforce", ~S"""
    **Hardware `may-use--<device>` tuples are documentation.** Bao's API
    ...
    details "Membership provisioning + revocation lifecycle", ~S"""
    ...
    **Revocation** (removing an agent):
    ...
    details "Bootstrap that landed with this RFD", ~S"""
    ...
    details "Idempotency and the reconciler as anti-entropy", ~S"""
    ...
    details "Reference invocation", ~S"""
