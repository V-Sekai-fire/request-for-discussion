...
#
# RFD 1169. `mix rfd.render` in rfd_dsl/ renders rfd/1169-the-audio-tower-for-qwen3-vl/README.md and
# DETAILS.md from this file; the Markdown is a build artifact (RFD 2232).
defmodule RFD1169 do
  use RFD.DSL

  rfd 1169, "The audio tower for Qwen3-VL, and which half compiles" do
    state :ideation
    ...
    **A voice is available; it is the acceleration that is not.**
    ...
    details_title "The audio tower for Qwen3-VL, and which half compiles"

    details "Why this is the vision tower again", ~S"""
    ...
    details "The operator question, which is the encouraging one", ~S"""
    ...
    **The input rank is the risk worth naming, and it looks survivable.**
    ...
    details "The fork gap, stated precisely", ~S"""
    ...
    **This is a change to a repository we own.** `weftspun/llama-cpp-npu-vision`
    ...
    details "ASR against voice cloning, and the correction", ~S"""
    ...
    **So the corrected statement is that the accelerator buys ears, and the
    ...
    details "Qwen3.5-Omni has no checkpoint to take a projector from", ~S"""
    ...
    details "The better answer for ASR is a smaller model, not a bigger tower", ~S"""
    ...
    **The first costs a download and the second costs a training run.**
    ...
    details "The checkpoint: licence clean, but the projector does not transfer", ~S"""
    ...
    **That is exactly EditScore's arrangement.** RFD 1157 records EditScore
    ...
    details "The fixed window, which is unresolved", ~S"""
    ...
    details "What to measure, cheapest first", ~S"""
