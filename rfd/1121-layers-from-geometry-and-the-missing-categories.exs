...
#
# RFD 1121. `mix rfd.render` in rfd_dsl/ renders rfd/1121-layers-from-geometry-and-the-missing-categories/README.md and
...
defmodule RFD1121 do
  ...
  rfd 1121,
    ...
    **Half B is the missing half.** Build the geometry first and render it second; the order is the
    ...
    details_title "Layers come from geometry, and the missing categories come from a multiview rebuild"

    details "What hm08 actually covers", ~S"""
    ...
    **And the original sentence is true.** Three top-level groups in `basemesh_vertex_groups.json`
    ...
    details "The state of that repository", ~S"""
    ...
    details "The 24 tags, against those groups", ~S"""
    ...
    details "The basemesh vertex order is frozen", ~S"""
    ...
    **State the coverage honestly.** The face array references vertex indices up to 14741 only, so
    ...
    details "Why the depth order needs no inpainting", ~S"""
    ...
    details "The rebuild, stage by stage", ~S"""
    ...
    details "What the corpus render writes", ~S"""
    ...
    details "Gates, each with a negative control", ~S"""
    ...
    details "Garments come from geometry too, and cloth-fit is how they get there", ~S"""
