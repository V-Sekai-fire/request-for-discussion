...
#
# RFD 1073. `mix rfd.render` in rfd_dsl/ renders rfd/1073-dataset-billboard-gallery/README.md and
# DETAILS.md from this file; the Markdown is a build artifact (RFD 2232).
defmodule RFD1073 do
  use RFD.DSL

  rfd 1073, "A billboard gallery of the RFD 1064 dataset, in one USD stage" do
    state :prediscussion
    ...
    details_title "A billboard gallery of the RFD 1064 dataset, in one USD stage"

    details "What shipped, verified, not claimed", ~S"""
    ...
    details "What replaced it: versitygw, verified end to end", ~S"""
    ...
    **RFD 1077 reverses this choice**, back to Tigris, once RFD 1076
    ...
    details "Two real findings after a browser, not curl, checked the site", ~S"""
    ...
    **`versitygw test full-flow` took the whole app down, for real.**
    ...
    details "A real upstream bug in usd-viewer, filed, not just patched", ~S"""
    ...
    details "A second real usd-viewer bug: no sRGB decode", ~S"""
    ...
    details "A third real usd-viewer bug: single-sided once a real texture applies", ~S"""
    ...
    details "What full scale still needs", ~S"""
    ...
    details "The gallery's asset is still a stopgap, decided which shape replaces it", ~S"""
